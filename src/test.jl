...
# x = y = param(rand(10))
# @test gradtest((x, y) -> x .+ x.^y.^x .* y .- x, ones(5), 2ones(5))

# haha(x) = sin(x)
# foo1(x, y) = Tracker.gradient((x, y) -> sum(x .+ 2f0 .* sin.(y).^x ./ x .-x .- x), x, y)
# foo2(x, y) = Tracker.gradient((x, y) -> sum(x .+ 2f0 .* haha.(y).^x ./ x .-x .- x), x, y)
# x = y = ones(Float32, 1000000)
# @time foo1(x, y);
# @time foo2(x, y);
